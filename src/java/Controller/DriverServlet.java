...
public class DriverServlet extends HttpServlet {
   protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    boolean requestSent = false; // Default value

    try {
        requestSent = DriverDAO.registerDriver(userId, licenseNumber, vehicleType, vehicleNumber);
    } catch (Exception e) { // Catch SQLException and other potential exceptions
        e.printStackTrace();
    ...
    if (requestSent) {
        session.setAttribute("message", "Your request has been sent to the admin for approval.");
    } else {
        session.setAttribute("error", "Failed to send request. Please try again.");
    }
    response.sendRedirect("UsersDashboard.jsp");
}
}
...
//        String vehicleNumber = request.getParameter("vehicleNumber");
//
//        boolean requestSent = userService.sendDriverRequest(userId, license, vehicleType, vehicleNumber);
//
//        if (requestSent) {
//            response.sendRedirect("ManageDrivers.jsp?msg=Request sent successfully!");
//        } else {
//            response.sendRedirect("UsersDashboard.jsp?msg=Error sending request!");
//        }
//    }
//}
